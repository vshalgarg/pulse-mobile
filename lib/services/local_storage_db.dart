...
class LocalStorageDB {
  ...
  static String? get getUserId => LocalStorageService.getString(LocalStorageConstants.userId);

  static String? get getFireBaseToken => LocalStorageService.getString(LocalStorageConstants.firebaseToken);

  static String? get getEmail => LocalStorageService.getString(LocalStorageConstants.email);

  static String? get getToken => LocalStorageService.getString(LocalStorageConstants.token);
  ...
  static DateTime? get getTokenExpiry {
    final expiryString = LocalStorageService.getString(LocalStorageConstants.tokenExpiry);
  ...
  static Future<void> saveTokenExpiry(DateTime expiry) async {
    await LocalStorageService.setString(LocalStorageConstants.tokenExpiry, expiry.toIso8601String());
  }

  static Future<void> saveUsername(String username) async {
    await LocalStorageService.setString(LocalStorageConstants.username, username);
  }

  static Future<void> savePassword(String password) async {
    await LocalStorageService.setString(LocalStorageConstants.password, password);
  }

  static Future<void> setRememberMe(bool rememberMe) async {
    await LocalStorageService.setBool(LocalStorageConstants.rememberMe, rememberMe);
  }

  static String? get getUsername => LocalStorageService.getString(LocalStorageConstants.username);

  static String? get getPassword => LocalStorageService.getString(LocalStorageConstants.password);

  static bool get getRememberMe => LocalStorageService.getBool(LocalStorageConstants.rememberMe) ?? false;

  static String? get getCartCount => LocalStorageService.getString(LocalStorageConstants.cartCount);

  static Future<void> setCartCount(int? cartCount) async {
    await LocalStorageService.setInt(LocalStorageConstants.cartCount, cartCount ?? 0);
  }

  // Profile Management
  static String? get getProfileImage => LocalStorageService.getString(LocalStorageConstants.profileImage);

  static String? get getFirstName => LocalStorageService.getString(LocalStorageConstants.firstName);

  static Future<void> saveFullName(String fullName) async {
    await LocalStorageService.setString(LocalStorageConstants.fullName, fullName);
  }

  static String? get getFullName => LocalStorageService.getString(LocalStorageConstants.fullName);

  static String? get appSessionKey => LocalStorageService.getString('sessionKey');

  // Energy Reading Form Data Methods
  static Future<void> saveEnergyReadingFormData(Map<String, dynamic> formData) async {
    await LocalStorageService.setJson(LocalStorageConstants.energyReadingFormData, formData);
  }

  static Map<String, dynamic>? get getEnergyReadingFormData {
    try {
      return LocalStorageService.getJson(LocalStorageConstants.energyReadingFormData);
    } catch (e) {
  ...
  static Future<void> saveEnergyReadingIds({
    ...
    await LocalStorageService.setString(LocalStorageConstants.energyReadingAuditSchId, auditSchId);
    await LocalStorageService.setString(LocalStorageConstants.energyReadingSiteAuditSchId, siteAuditSchId);
    await LocalStorageService.setString(LocalStorageConstants.energyReadingSiteId, siteId);
  }

  static String? get getEnergyReadingAuditSchId => 
      LocalStorageService.getString(LocalStorageConstants.energyReadingAuditSchId);
  
  static String? get getEnergyReadingSiteAuditSchId => 
      LocalStorageService.getString(LocalStorageConstants.energyReadingSiteAuditSchId);
  ...
  static Future<void> clearEnergyReadingData() async {
    await LocalStorageService.remove(LocalStorageConstants.energyReadingFormData);
    await LocalStorageService.remove(LocalStorageConstants.energyReadingAuditSchId);
    await LocalStorageService.remove(LocalStorageConstants.energyReadingSiteAuditSchId);
  ...
  static Future<void> saveAssetAuditSelfie({
    ...
    print('LocalStorageDB: Saved selfie for site $siteAuditSchId with image ID $imageId');
  }
  ...
  static Future<void> updateAssetAuditSelfie({
    ...
    print('LocalStorageDB: Updated selfie for site $siteAuditSchId with new image ID $newImageId');
  }
  ...
  static Future<void> saveAssetAuditFormData({
    ...
    try {
      final key = '${LocalStorageConstants.assetAuditFormDataKey}${siteAuditSchId}_$screenName';
      await LocalStorageService.setJson(key, {
        'formData': formData,
        'timestamp': DateTime.now().millisecondsSinceEpoch,
      ...
      print('LocalStorageDB: Saved form data for screen $screenName, site $siteAuditSchId');
    } catch (e) {
  ...
  static Map<String, dynamic>? getAssetAuditFormData({
    ...
    try {
      ...
      final data = LocalStorageService.getJson(key);
      if (data != null) {
        print('LocalStorageDB: Retrieved form data for screen $screenName, site $siteAuditSchId');
        return data;
      }
      return null;
    } catch (e) {
  ...
  static Future<void> updateAssetAuditFormData({
    ...
    try {
      ...
      print('LocalStorageDB: Updated form data for screen $screenName, site $siteAuditSchId');
    } catch (e) {
  ...
  static Future<void> clearAssetAuditFormData({
    ...
    final key = '${LocalStorageConstants.assetAuditFormDataKey}${siteAuditSchId}_$screenName';
    await LocalStorageService.remove(key);
    print('LocalStorageDB: Cleared form data for screen $screenName, site $siteAuditSchId');
  }

  static Future<void> clearAllAssetAuditFormData(String siteAuditSchId) async {
    final keys = LocalStorageService.getKeys();
    final prefix = '${LocalStorageConstants.assetAuditFormDataKey}$siteAuditSchId';
  ...
  static Map<String, dynamic>? getOfflineTicket(String siteAuditSchId) {
    try {
      ...
      final data = LocalStorageService.getJson(key);
      if (data != null) {
        print('LocalStorageDB: Retrieved offline ticket for site $siteAuditSchId');
        return data;
      }
      return null;
    } catch (e) {
  ...
  static Future<void> logout() async {
    print("LocalStorageDB: Starting logout process - clearing all authentication data");
  ...
  static Future<void> clearAllCredentials() async {
    print("LocalStorageDB: Clearing all credentials including remember me data");
    await LocalStorageService.remove(LocalStorageConstants.username);
    await LocalStorageService.remove(LocalStorageConstants.password);
    await LocalStorageService.remove(LocalStorageConstants.rememberMe);
    await LocalStorageService.remove(LocalStorageConstants.token);
  ...
  static Map<String, String> getHeadersWithToken() {
    return {
      "Authorization": 'Bearer ${LocalStorageDB.getToken}',
    };
  }
