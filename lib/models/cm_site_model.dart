class CMSite {
  ...
  factory CMSite.fromJson(Map<String, dynamic> json) {
    ...
    print('🔄 [CMSite] Parsed values:');
    print('   - siteId: $siteId (${siteId.runtimeType})');
    print('   - siteName: "$siteName" (${siteName.runtimeType})');
    print('   - siteCode: "$siteCode" (${siteCode.runtimeType})');
    
    // Validate required fields
    if (siteId == 0) {
      print('⚠️ [CMSite] Warning: site_id is 0 or null');
    }
    if (siteName.isEmpty) {
      print('⚠️ [CMSite] Warning: site_name is empty');
    }
    if (siteCode.isEmpty) {
      print('⚠️ [CMSite] Warning: site_code is empty');
    }
