...
class _PmSolarPage7State extends State<PmSolarPage7> {
  ...
  String _getSuccessMessage() {
    ...
    return "Inverters section for Solar Site (ID: $siteId) has been recorded and saved.";
  }

  String _getCancelMessage() {
    final siteId = _getActualSiteId();
    return "Do you want to cancel the Inverters section for Solar Site (ID: $siteId) ?";
  }
  ...
  void _loadExistingData(PmGetDataModel data) {
    print('=== Loading Existing Data for PM Solar Page 7 (Inverters) ===');
    setState(() {
      formData.clear();
      photoIds.clear();
      ...
      // Load Inverters section data
      final invertersData = data.responseData?.inverters ?? [];
      print('Inverters data count: ${invertersData.length}');
      print('Raw Inverters data: $invertersData');

      for (final item in invertersData) {
  ...
  bool _isFieldEditable(dynamic item) {
    ...
    final invertersData = widget.pmData?.responseData?.inverters ?? [];
    final bool isEditable = _checkInvertersDependencies(item, invertersData);

    return isEditable;
  }

  bool _checkInvertersDependencies(dynamic item, List<dynamic> sectionData) {
  ...
  Future<void> _submitForm() async {
    ...
    print('Total available fields: ${widget.pmData?.responseData?.inverters?.length ?? 0}');
    ...
    return MultiBlocListener(
      listeners: [
        BlocListener<PmCubit, PmState>(
          listener: (context, state) {
            if (state is PmGetLoaded) {
              print('PmGetLoaded triggered with Inverters data: ${state.pmGetDataModel.responseData?.inverters}');
                ...
                Expanded(
                  child: SingleChildScrollView(
                    ...
                    child: Container(
                      ...
                      child: _buildInvertersSection(data),
                    ),
                  ),
                ),
                Container(
                        ...
                        child: ArrowButton(
                          ...
                          onPressed: isSubmitting
                            ...
                            if (formData.isNotEmpty) {
                              ...
                              if (state is PmPostSuccess && !isSubmitting) {
                                print('Submission successful, navigating to PmSolarPage8');
                                _navigateToPage8();
                              } else if (state is PmPostError) {
                                ...
                                if (context.read<PmCubit>().state is PmPostSuccess && !isSubmitting) {
                                  print('Delayed navigation to PmSolarPage8');
                                  _navigateToPage8();
                                } else {
                                  ScaffoldMessenger.of(context).showSnackBar(
                              ...
                              print('No data to submit, navigating to PmSolarPage8');
                              _navigateToPage8();
                            }
                          },
                        ),
