...
class _PmSolarPage1State extends State<PmSolarPage1> {
  ...
  Widget _buildEarthingSection(PmGetDataModel data) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        _buildFormField('Check condition of earth pits', 'DROPDOWN,IMG'),
        getHeight(15),
        _buildFormField(
          'Check for earthing value (should be less than 2 Volts)',
          'DROPDOWN,IMG',
        ),
        getHeight(15),
        _buildFormField('EGB Status', 'DROPDOWN,IMG'),
        getHeight(15),
        _buildFormField('LA status & LA Strip connection', 'DROPDOWN,IMG'),
        getHeight(15),
        _buildFormField('Any rectification remarks', 'TEXT'),
        getHeight(15),
      ],
    );
  }
  ...
  Widget _buildFormField(String label, String respType) {
    ...
    if (respType.contains('DROPDOWN') && respType.contains('IMG')) {
              ...
              children: [
                CustomDropdown(
                  ...
                  onChanged: isEditable
                      ? (value) => _saveFormData(label, value)
                      : (_) {},
                  isRequired: true,
                ),
                getHeight(15),
                ImageUploadField(
                  ...
                  onImageSelected: isEditable
                      ? (image) => _saveFormData('${label}_image', image)
                      : (image) {},
                  isRequired: true,
                ),
              ],
      ...
      return ImageUploadField(
        label: label,
        onImageSelected: isEditable
            ? (image) => _saveFormData(label, image)
            : (image) {},
        isRequired: true,
      );
  ...
  String _getSuccessMessage() {
    return 'Earthing section data saved successfully!';
  }

  String _getCancelMessage() {
    return 'Earthing section data cancelled!';
  }

  String _getButtonText() {
    return 'Solar'; // Shorter text to ensure button fits
  }
}
