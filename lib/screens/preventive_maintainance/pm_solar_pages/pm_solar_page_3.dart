...
class PmSolarPage3 extends StatefulWidget {
  ...
  @override
  State<PmSolarPage3> createState() => _PmSolarPage3State();
}

class _PmSolarPage3State extends State<PmSolarPage3> {
  ...
  String _getSuccessMessage() {
    ...
    return "BOS section for Solar Site (ID: $siteId) has been recorded and saved.";
  }

  String _getCancelMessage() {
    final siteId = _getActualSiteId();
    return "Do you want to cancel the BOS section for Solar Site (ID: $siteId) ?";
  }

  String _getActualSiteId() {
    // Prioritize widget.siteId to avoid context issues
  ...
  void _loadExistingData(PmGetDataModel data) {
    print('=== Loading Existing Data for PM Solar Page 3 (BOS) ===');
    setState(() {
      formData.clear();
      photoIds.clear();
      ...
      final bosData = data.responseData?.bos ?? [];
      print('BOS data count: ${bosData.length}');
      print('Raw BOS data: $bosData');

      for (final item in bosData) {
  ...
  bool _isFieldEditable(dynamic item) {
    ...
    final bosData = widget.pmData?.responseData?.bos ?? [];
    final bool isEditable = _checkBOSDependencies(item, bosData);
    return isEditable;
  }

  bool _checkBOSDependencies(dynamic item, List<dynamic> sectionData) {
  ...
  Future<void> _submitForm() async {
    ...
    print('Total available fields: ${widget.pmData?.responseData?.bos?.length ?? 0}');
      ...
      listeners: [
        BlocListener<PmCubit, PmState>(
          listener: (context, state) {
            ...
            if (state is PmGetLoaded) {
              print('PmGetLoaded triggered with BOS data: ${state.pmGetDataModel.responseData?.bos}');
                          ...
                          onPressed: isSubmitting
                            ...
                            if (formData.isNotEmpty) {
                              ...
                              if (state is PmPostSuccess) {
                                print('Submission successful, navigating to PmSolarPage4');
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                              ...
                              print('No data to submit, navigating to PmSolarPage4');
                              Navigator.push(
                                context,
                                MaterialPageRoute(
                                  builder: (context) => PmSolarPage4(
  ...
  Widget _buildBOSSection(PmGetDataModel data) {
    final bosData = data.responseData?.bos ?? [];
    print('Building BOS section with data: $bosData');
  ...
  Widget _buildFormField(dynamic item) {
    ...
    if (respType.contains('TEXT')) {
      ...
      return CustomFormField(
        ...
        onChanged: (value) {
          print('Remarks field changed: $key = $value');
          _saveFormData(key, value);
        },
      );
    }
