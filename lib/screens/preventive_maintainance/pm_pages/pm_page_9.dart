...
import 'package:app/commonWidgets/custom_dropdown.dart' hide CustomDropdown;
...
import 'package:app/screens/preventive_maintainance/pm_pages/pm_page_10.dart';
...
import '../../../commonWidgets/custom_form_dropdown.dart' hide CustomDropdown;
...
class PmScreen9 extends StatefulWidget {
  ...
  @override
  State<PmScreen9> createState() => _PmScreen9();
}


class _PmScreen9 extends State<PmScreen9> {
  ...
  String _getPmTitle() {
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "PM Telecom - Battery";
      case PmTicketTypeEnum.solar:
        return "PM Solar - Battery";
    }
  }
  ...
  String _getSuccessMessage() {
    ...
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "Battery section for Telecom Site (ID: $siteId) has been recorded and saved.";
      case PmTicketTypeEnum.solar:
        return "Battery section for Solar Site (ID: $siteId) has been recorded and saved.";
    }
  }
  ...
  String _getCancelMessage() {
    ...
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "Do you want to cancel the Battery section for Telecom Site (ID: $siteId) ?";
      case PmTicketTypeEnum.solar:
        return "Do you want to cancel the Battery section for Solar Site (ID: $siteId) ?";
    }
  }
  ...
  void _loadExistingData(PmGetDataModel data) {
    print('=== Loading Existing Data for PM Page 9 ===');
    setState(() {
      formData.clear();
      photoIds.clear();
      ...
      final batteryData = data.responseData?.battery ?? [];
      print('Battery data count: ${batteryData.length}');
      print('Raw battery data: $batteryData');
  ...
  bool _isFieldEditable(dynamic item) {
    ...
    if (respType.contains('DROPDOWN') || respType.contains('RADIO')) {
      return true;
    }

    final batteryData = widget.pmData?.responseData?.battery ?? [];
    final bool isEditable = _checkBatteryDependencies(item, batteryData);

    return isEditable;
  }

  bool _checkBatteryDependencies(dynamic item, List<dynamic> sectionData) {
  ...
  Widget _buildBatterySection(PmGetDataModel data) {
    final batteryData = data.responseData?.battery ?? [];
          ...
          listeners: [
            BlocListener<PmCubit, PmState>(
              listener: (context, state) {
                if (state is PmGetLoaded) {
                  print('PmGetLoaded triggered with Battery data: ${state.pmGetDataModel.responseData?.battery}');
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Container(
                    ...
                    child: _buildBatterySection(data),
                  ),
                ),
              ),
              Container(
