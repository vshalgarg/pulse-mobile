...
class PmScreen12 extends StatefulWidget {
  ...
  @override
  State<PmScreen12> createState() => _PmScreen12();
}

class _PmScreen12 extends State<PmScreen12> {
  ...
  String _getSuccessMessage() {
    ...
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "Preventive Maintenance for Telecom Site (ID: $siteId) has been completed and saved.";
      case PmTicketTypeEnum.solar:
        return "Preventive Maintenance for Solar Site (ID: $siteId) has been completed and saved.";
    }
  }
  ...
  String _getCancelMessage() {
    ...
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "Do you want to cancel the Hygiene section for Telecom Site (ID: $siteId) ?";
      case PmTicketTypeEnum.solar:
        return "Do you want to cancel the Hygiene section for Solar Site (ID: $siteId) ?";
    }
  }
  ...
  void _loadExistingData(PmGetDataModel data) {
    ...
    print('Hygiene data count: ${data.responseData?.hygiene?.length ?? 0}');
    setState(() {
      formData.clear();
      photoIds.clear();
      ...
      for (final item in hygieneData) {
        final key = '${item.pmItemType}_${item.clOrder}';
        print(
          'Item: $key, checklist_desc: ${item.checklistDesc}, photo_id: ${item.photoId}, resp: ${item.resp}, resp_type: ${item.respType}',
        );
        ...
        if (item.resp != null) {
          ...
          print('Form data added: $key = $mappedValue');
        }
        ...
        if (item.photoId != null) {
          ...
          if (photoIdInt != null) {
            ...
            print('Photo ID added: $key = $photoIdInt');
        ...
        if (item.photoTakenTs != null) {
          ...
          print('Timestamp added: $key = ${item.photoTakenTs}');
        }
      ...
      print(
        'Final state: formData=$formData, photoIds=$photoIds, photoTimestamps=$photoTimestamps, loadedImageUrls=$loadedImageUrls, retryCounts=$_retryCounts',
      );
      _dummyState = DateTime.now().millisecondsSinceEpoch;
    });
  ...
  void _loadImageForPhotoId(String photoId, String key) {
    ...
    _imageRequestKeys[photoId] =
        key; // Store the mapping between photoId and key
  ...
  bool _isFieldEditable(dynamic item) {
    ...
    // Check for specific dependencies in the Hygiene section
    final hygieneData = widget.pmData?.responseData?.hygiene ?? [];
    final bool isEditable = _checkHygieneDependencies(item, hygieneData);

    return isEditable;
  }

  bool _checkHygieneDependencies(dynamic item, List<dynamic> sectionData) {
    final itemDesc = item.checklistDesc?.toLowerCase() ?? '';

    // Enable "Any rectification remarks" if any DROPDOWN field is "Not OK - To be corrected" or "Corrected"
    if (itemDesc.contains('rectification remarks') ||
        item.respType?.contains('REMARKS') == true) {
      for (final dropdownItem in sectionData) {
        if (dropdownItem.respType?.contains('DROPDOWN') == true) {
          final dropdownKey =
              '${dropdownItem.pmItemType}_${dropdownItem.clOrder}';
          final dropdownValue = formData[dropdownKey];

          if (dropdownValue == 'Not OK - To be corrected' ||
              dropdownValue == 'Corrected') {
            return true; // Enable editing for rectification remarks
          }
        }
      }
      return false; // Disable if no "Not OK" or "Corrected" found
    }
  ...
  void _showCompletionDialog() {
    showDialog(
      ...
      builder: (context) => SuccessDialog(
        ticketId: "PM-${DateTime.now().millisecondsSinceEpoch}",
        message: _getSuccessMessage(),
        onDone: () {
          Navigator.of(context).pop(); // Close dialog
          Navigator.of(
            context,
          ).popUntil((route) => route.isFirst); // Navigate to ticket screen
        },
      ),
    );
  }
  ...
  Widget _buildFormField(dynamic item) {
    ...
    print(
      'Processing item with respType: ${item.respType}, checklistDesc: ${item.checklistDesc}, externalImageUrl: ${loadedImageUrls[key]}',
    );
    ...
    if (respType.contains('TEXT')) {
      ...
      if (respType.contains('IMG') || photoIds[key] != null) {
        return Column(
    ...
    } else if (respType.contains('REMARKS') ||
          ...
          listeners: [
            ...
            BlocListener<PmCubit, PmState>(
                    listener: (context, state) {
                      if (state is PmGetLoaded) {
                        print(
                          'PmGetLoaded triggered with Hygiene data: ${state.pmGetDataModel.responseData?.hygiene}',
                        );
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              'Error saving data: ${state.message}',
                            ),
                          ),
                        );
                    ...
                    listener: (context, state) {
                      if (state is AssetAuditPhotoUploadSuccess) {
                        if (_currentUploadKey != null) {
                          final photoId =
                              int.tryParse(state.response.imgId) ?? 0;
                          ...
                          print(
                            'Photo uploaded: photoId=$photoId, timestamp=$timestamp for key=$_currentUploadKey',
                          );
                        }
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              'Photo upload failed: ${state.errorMessage}',
                            ),
                          ),
                        );
                      }
                  ...
                  BlocListener<
                    AssetAuditGetImageCubit,
                    AssetAuditGetImageState
                  >(
                    listener: (context, state) async {
                      if (state is AssetAuditGetImageSuccess) {
                        final imageData = state.imageData;
                        print(
                          'Image loaded for photoId: $_lastRequestedPhotoId, data length: ${imageData.length}, data: ${imageData.substring(0, imageData.length > 50 ? 50 : imageData.length)}...',
                        );
                          ...
                          if (key != null) {
                            ...
                            if (imageData.isNotEmpty) {
                              setState(() {
                                ...
                                print(
                                  'Updated loadedImageUrls: $key = ${imageData.substring(0, imageData.length > 50 ? 50 : imageData.length)}...',
                                );
                              });
                              ...
                              print(
                                'Empty imageData for photoId: $_lastRequestedPhotoId',
                              );
                            ...
                            print(
                              'No key found for photoId: $_lastRequestedPhotoId',
                            );
                          }
                        ...
                        print(
                          'Image load failed for photoId: $_lastRequestedPhotoId, error: ${state.errorMessage}',
                        );
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                        ...
                        if (_lastRequestedPhotoId != null) {
                          ...
                          if (key != null) {
                            ...
                            print(
                              'No key found for failed photoId: $_lastRequestedPhotoId',
                            );
                          }
                          _imageRequestKeys.remove(_lastRequestedPhotoId);
                          _lastRequestedPhotoId = null;
              ...
              Expanded(
                child: SingleChildScrollView(
                  ...
                  child: Container(
                    ...
                    child: _buildHygieneSection(data),
                  ),
                ),
              ),
              Container(
