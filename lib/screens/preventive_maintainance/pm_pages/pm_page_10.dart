...
import 'package:app/screens/preventive_maintainance/pm_pages/pm_page_11.dart';
...
class PmScreen10 extends StatefulWidget {
  ...
  @override
  State<PmScreen10> createState() => _PmScreen10();
}


class _PmScreen10 extends State<PmScreen10> {
  ...
  String _getPmTitle() {
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "PM Telecom - Tower";
      case PmTicketTypeEnum.solar:
        return "PM Solar - Tower";
    }
  }
  ...
  String _getSuccessMessage() {
    ...
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "Tower section for Telecom Site (ID: $siteId) has been recorded and saved.";
      case PmTicketTypeEnum.solar:
        return "Tower section for Solar Site (ID: $siteId) has been recorded and saved.";
    }
  }
  ...
  String _getCancelMessage() {
    ...
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "Do you want to cancel the Tower section for Telecom Site (ID: $siteId) ?";
      case PmTicketTypeEnum.solar:
        return "Do you want to cancel the Tower section for Solar Site (ID: $siteId) ?";
    }
  }
  ...
  void _loadExistingData(PmGetDataModel data) {
    print('=== Loading Existing Data for PM Page 10 ===');
    setState(() {
      formData.clear();
      photoIds.clear();
      ...
      final towerData = data.responseData?.tower ?? [];
      print('Tower data count: ${towerData.length}');
      print('Raw tower data: $towerData');

      for (final item in towerData) {
          ...
          if (item.resp is int) {
            ...
            if (item.respType?.contains('DROPDOWN') ?? false) {
              ...
              try {
                ...
                mappedValue = '';
                print('Failed to map resp to dropdown option: $respValue, setting to empty');
              }
            ...
            if (item.respType?.contains('DROPDOWN') ?? false) {
              ...
              if (dropdownOptions.contains(item.resp.toString())) {
                mappedValue = item.resp.toString();
              } else {
                mappedValue = '';
                print('Invalid resp for DROPDOWN: ${item.resp}, setting to empty');
              }
            } else {
              mappedValue = item.resp.toString();
            }
  ...
  bool _isFieldEditable(dynamic item) {
    ...
    final towerData = widget.pmData?.responseData?.tower ?? [];
    final bool isEditable = _checkTowerDependencies(item, towerData);

    return isEditable;
  }

  bool _checkTowerDependencies(dynamic item, List<dynamic> sectionData) {
  ...
  Widget _buildFormField(dynamic item) {
    ...
    if (respType.contains('TEXT')) {
      ...
      const List<String> options = ['OK', 'Corrected', 'Not OK - To be corrected', 'Not Applicable'];
      if (respType.contains('IMG')) {
              ...
              child: Column(
                ...
                children: [
                  CustomDropdown(
                    ...
                    initialValue: isEditable && formData[key] != null && options.contains(formData[key]?.toString()) ? formData[key]?.toString() : null,
        ...
        return Container(
          ...
          child: CustomDropdown(
            label: checklistDesc.isNotEmpty ? checklistDesc : 'Select Option',
            items: isEditable ? options : [],
            initialValue: isEditable && formData[key] != null && options.contains(formData[key]?.toString()) ? formData[key]?.toString() : null,
            onChanged: isEditable ? (value) => _saveFormData(key, value) : (_) {},
            // isRequired: false,
  ...
  Widget _buildTowerSection(PmGetDataModel data) {
    final towerData = data.responseData?.tower ?? [];
        ...
        body: widget.pmData != null
          ...
          listeners: [
            ...
            BlocListener<PmCubit, PmState>(
              listener: (context, state) {
                if (state is PmGetLoaded) {
                  print('PmGetLoaded triggered with Tower data: ${state.pmGetDataModel.responseData?.tower}');
