...
import 'package:app/screens/preventive_maintainance/pm_pages/pm_page_4.dart';
...
class PmScreen3 extends StatefulWidget {
  ...
  @override
  State<PmScreen3> createState() => _PmScreen3();
}

class _PmScreen3 extends State<PmScreen3> {
  ...
  String _getSuccessMessage() {
    ...
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "Earthing section for Telecom Site (ID: $siteId) has been recorded and saved.";
      case PmTicketTypeEnum.solar:
  ...
  String _getCancelMessage() {
    ...
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "Do you want to cancel the Earthing section for Telecom Site (ID: $siteId) ?";
      case PmTicketTypeEnum.solar:
  ...
  void _loadExistingData(PmGetDataModel data) {
    print('=== Loading Existing Data for PM Page 3 ===');
    setState(() {
      formData.clear();
      photoIds.clear();
    ...
    if (itemDesc.contains('rectification remarks')) {
      for (final dropdownItem in sectionData) {
        if (dropdownItem.respType?.contains('DROPDOWN') == true &&
            dropdownItem.checklistDesc?.toLowerCase().contains('condition of earth pits') == true) {
