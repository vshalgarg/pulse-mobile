...
import 'package:app/screens/preventive_maintainance/pm_pages/pm_page_8.dart';
...
class PmScreen7 extends StatefulWidget {
  ...
  @override
  State<PmScreen7> createState() => _PmScreen7();
}


class _PmScreen7 extends State<PmScreen7> {
  ...
  String _getPmTitle() {
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "PM Telecom - Solar";
      case PmTicketTypeEnum.solar:
        return "PM Solar - Solar";
    }
  }
  ...
  String _getSuccessMessage() {
    ...
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "Solar section for Telecom Site (ID: $siteId) has been recorded and saved.";
      case PmTicketTypeEnum.solar:
        return "Solar section for Solar Site (ID: $siteId) has been recorded and saved.";
    }
  }
  ...
  String _getCancelMessage() {
    ...
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "Do you want to cancel the Solar section for Telecom Site (ID: $siteId) ?";
      case PmTicketTypeEnum.solar:
        return "Do you want to cancel the Solar section for Solar Site (ID: $siteId) ?";
    }
  }
  ...
  void _loadExistingData(PmGetDataModel data) {
    print('=== Loading Existing Data for PM Page 7 ===');
    setState(() {
      formData.clear();
      photoIds.clear();
      ...
      final solarData = data.responseData?.solar ?? [];
      print('Solar data count: ${solarData.length}');
      print('Raw solar data: $solarData');

      for (final item in solarData) {
  ...
  bool _isFieldEditable(dynamic item) {
    ...
    final solarData = widget.pmData?.responseData?.solar ?? [];
    final bool isEditable = _checkSolarDependencies(item, solarData);

    return isEditable;
  }

  bool _checkSolarDependencies(dynamic item, List<dynamic> sectionData) {
  ...
  Widget _buildSolarSection(PmGetDataModel data) {
    final solarData = data.responseData?.solar ?? [];
        ...
        body: widget.pmData != null
          ...
          listeners: [
            ...
            BlocListener<PmCubit, PmState>(
              listener: (context, state) {
                if (state is PmGetLoaded) {
                  print('PmGetLoaded triggered with Solar data: ${state.pmGetDataModel.responseData?.solar}');
