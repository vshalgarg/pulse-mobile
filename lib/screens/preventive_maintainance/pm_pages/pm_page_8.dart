...
import 'package:app/screens/preventive_maintainance/pm_pages/pm_page_9.dart';
...
class PmScreen8 extends StatefulWidget {
  ...
  @override
  State<PmScreen8> createState() => _PmScreen8();
}


class _PmScreen8 extends State<PmScreen8> {
  ...
  String _getSuccessMessage() {
    ...
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "CCU section for Telecom Site (ID: $siteId) has been recorded and saved.";
      case PmTicketTypeEnum.solar:
        return "CCU section for Solar Site (ID: $siteId) has been recorded and saved.";
    }
  }
  ...
  String _getCancelMessage() {
    ...
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "Do you want to cancel the CCU section for Telecom Site (ID: $siteId) ?";
      case PmTicketTypeEnum.solar:
        return "Do you want to cancel the CCU section for Solar Site (ID: $siteId) ?";
    }
  }
  ...
  void _loadExistingData(PmGetDataModel data) {
    print('=== Loading Existing Data for PM Page 8 ===');
    setState(() {
      formData.clear();
      photoIds.clear();
      ...
      final ccuData = data.responseData?.ccu ?? [];
      print('CCU data count: ${ccuData.length}');
      print('Raw CCU data: $ccuData');

      for (final item in ccuData) {
  ...
  bool _isFieldEditable(dynamic item) {
    ...
    final ccuData = widget.pmData?.responseData?.ccu ?? [];
    final bool isEditable = _checkCCUDependencies(item, ccuData);

    return isEditable;
  }

  bool _checkCCUDependencies(dynamic item, List<dynamic> sectionData) {
  ...
  Widget _buildCCUSection(PmGetDataModel data) {
    final ccuData = data.responseData?.ccu ?? [];
          ...
          listeners: [
            BlocListener<PmCubit, PmState>(
              listener: (context, state) {
                if (state is PmGetLoaded) {
                  print('PmGetLoaded triggered with CCU data: ${state.pmGetDataModel.responseData?.ccu}');
