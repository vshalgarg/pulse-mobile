...
import 'package:app/screens/preventive_maintainance/pm_pages/pm_page_5.dart';
...
class PmScreen4 extends StatefulWidget {
  ...
  @override
  State<PmScreen4> createState() => _PmScreen4();
}

class _PmScreen4 extends State<PmScreen4> {
  ...
  String _getSuccessMessage() {
    ...
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "SEB section for Telecom Site (ID: $siteId) has been recorded and saved.";
      case PmTicketTypeEnum.solar:
        return "SEB section for Solar Site (ID: $siteId) has been recorded and saved.";
    }
  }
  ...
  String _getCancelMessage() {
    ...
    switch (widget.ticketType) {
      case PmTicketTypeEnum.telecom:
        return "Do you want to cancel the SEB section for Telecom Site (ID: $siteId) ?";
      case PmTicketTypeEnum.solar:
        return "Do you want to cancel the SEB section for Solar Site (ID: $siteId) ?";
    }
  }
  ...
  void _loadExistingData(PmGetDataModel data) {
    print('=== Loading Existing Data for PM Page 4 ===');
    setState(() {
      formData.clear();
      photoIds.clear();
      ...
      final sebData = data.responseData?.seb ?? [];
      print('SEB data count: ${sebData.length}');
      print('Raw SEB data: $sebData');

      for (final item in sebData) {
  ...
  bool _isFieldEditable(dynamic item) {
    ...
    final sebData = widget.pmData?.responseData?.seb ?? [];
    final bool isEditable = _checkSEBDependencies(item, sebData);

    return isEditable;
  }

  bool _checkSEBDependencies(dynamic item, List<dynamic> sectionData) {
  ...
  Widget _buildSEBSection(PmGetDataModel data) {
    final sebData = data.responseData?.seb ?? [];

    if (sebData.isEmpty) {
      return const Center(
        child: Text(
          'No SEB data available',
        ...
        body: widget.pmData != null
          ...
          listeners: [
            ...
            BlocListener<PmCubit, PmState>(
              listener: (context, state) {
                if (state is PmGetLoaded) {
                  print('PmGetLoaded triggered with SEB data: ${state.pmGetDataModel.responseData?.seb}');
