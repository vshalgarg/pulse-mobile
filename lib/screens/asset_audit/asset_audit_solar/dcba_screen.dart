...
class _DCBAScreenState extends State<DCBAScreen> {
  ...
  Future<void> _postDCBAData() async {
    ...
    if (savedDcbaItems.isEmpty && remarksController.text.trim().isEmpty) {
      print('=== DCBA LOCAL SAVE: Nothing to save ===');
      return;
    }
    ...
    try {
      ...
      // ---- Save each DCDB row locally ----
      if (savedDcbaItems.isNotEmpty) {
        print('=== DCBA LOCAL SAVE: Processing ${savedDcbaItems.length} saved items ===');
        for (int i = 0; i < savedDcbaItems.length; i++) {
          print('Item $i: ${savedDcbaItems[i]}');
        }

        for (final item in savedDcbaItems) {
      ...
      if (mounted) {
        showCustomToast(context, saved > 0 ? 'DCBA saved locally ($saved)' : 'Nothing to save');
      }
      print('=== DCBA LOCAL SAVE: completed; rows=$saved ===');
    } catch (e, st) {
      debugPrint('DCBA local save failed: $e\n$st');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _postDCBAData_api() async {
