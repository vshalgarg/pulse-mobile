...
class _LTDBScreenState extends State<LTDBScreen> {
  ...
  Future<void> _postLtdbData() async {
    if (savedLtdbItems.isEmpty && remarksController.text.trim().isEmpty) {
      print('LTDB: Nothing to save');
      return;
    }
    ...
    try {
      ...
      // ---- Save each LTDB row locally ----
      for (final item in savedLtdbItems) {
        ...
        await assetAuditService.upsertFromRequest(req, 'solar_ltdb', localPhotoId);
        saved++;
      }
      ...
      if (remarksText.isNotEmpty) {
        ...
        await assetAuditService.upsertFromRequest(reqRemarks, 'solar_ltdb', "");
        saved++;
      }

      if (mounted) {
        showCustomToast(context, saved > 0 ? 'LTDB saved locally ($saved)' : 'Nothing to save');
      }
    } catch (e, st) {
      debugPrint('LTDB local save failed: $e\n$st');
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _postLtdbData_api() async {
    try {
      final assetAuditState = context.read<AssetAuditCubit>().state;
      if (assetAuditState is AssetAuditLoaded && assetAuditState.assetAuditData.pageHeader.isNotEmpty) {
